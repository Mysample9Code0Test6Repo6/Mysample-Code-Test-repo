...
<div id="container">
    ...
    <div id="leftPane">   
   	...
   	<div class="centerPane">        
        ...
        <div class="cpView">    
        <form:form  id="addGroupForm"  modelAttribute="group" method="post" commandName="group"> 
        		<center>
        		<c:if scope="request" test="${conMsg eq true }" var="conMsg">
            	<font face="times, serif" size="3" color="red">Please add contacts before you create a group.</font>
            </c:if>	
        		<table width="100%">
        			<tr align="center" height="40 px;">
        				<td><font face="times, serif" size="3"><u> <a href='<c:url value="/addgroup.cyt"/>'>Create Group</a></u></font></td>
        				<td><font face="times, serif" size="3"><u><a href='<c:url value="/searchgroupbyname.cyt"/>'>Search By Group Name</a></u></font></td>
        				<td><font face="times, serif" size="3"><u><a href='<c:url value="/searchgroupbycode.cyt"/>'>Search By Post Code</a></u></font></td>
        			</tr>
        			<tr height="20 px;"/>
        			<tr align="center" colspan="4" height="40 px;">
        				<td><b>Your created groups</b></td>
        			
        			
               		<c:forEach items="${userGroups}" var="group" varStatus="rowItem">
               		<form:hidden path="groupId"/>
               		<tr align="center" >
               			<td><form:checkbox path="selectedGroupIds" value="${group.groupId}" /></td>
               			<td><c:out value="${rowItem.index + 1 }" /></td>
               			<td><font face="bold"> <a href="#" onclick="displayGroupInfo('<c:url value='usergroups.cyt' />','${group.groupId }');"><c:out value="${group.groupName }" /></a></font></td>
               			<td>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="editGroup('<c:url value='editgrouppage.cyt' />','${group.groupId }');">EDIT</a></td>
               			<td>&nbsp;<a href='#' onclick="removeGroup('<c:url value='removegroup.cyt' />','${group.groupId }');"/>DELETE</a></td>
               		</tr>
               		</c:forEach>
               		<tr align="center" colspan="4" height="40 px;">
               			<td><input type="button" value="Delete Groups" onclick="deleteGroups('<c:url value='removegroups.cyt' />');"/></td>
               		</tr>
        		</table>
        ...
        <div class="cpTop">                
            ...
            <form:form  id="addCommentForm"  modelAttribute="forum" method="post" commandName="forum" > 
            <div class="bgBody">
                      <p class="panelFont">
                       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                         <form:textarea path="comment" cols="50" rows="2" style="overflow:hidden; width: 400px; font-size: 15px;
                         vertical-align: middle" class="name"  title="Post your coment here" />
                       <input type="button" value="Post" style="width:65px; height:40px; vertical-align:middle" onclick="addComment('<c:url value='addcomment.cyt' />');"/>
              </p>            
       	  </div>
       	  </form:form> 
