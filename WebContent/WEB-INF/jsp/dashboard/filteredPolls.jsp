...
<%@include file="/WEB-INF/jsp/login_model/filteredPollsModelLogin.jsp"%>
...
<% User userLocal1 = (User) session.getAttribute("user"); %>
...
<div id="most_resent_incyyte">
				<c:if test="${not empty text}">
					<p style="padding-bottom:50px; text-align:center;font: 50px/1em 'bariol_thinregular', 'ie8_bariol_thin';  "> Searched for 
						<span style="font: 1em/1em 'bariol_boldbold', 'ie8_bariol_bold';  color: #1B303F;">"${text}" </span> </p> 
				</c:if>
				<c:if test="${not empty category}">
					<p style="padding-bottom:50px; text-align:center;font: 50px/1em 'bariol_thinregular', 'ie8_bariol_thin';">
						<span style="font: 1em/1em 'bariol_boldbold', 'ie8_bariol_bold';  color: #1B303F;">${category} </span>Polls</p>
				</c:if>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  		<tr>
					    		...
					    		<td width="58%"  >
			      				...
			      				<td width="30%" >
					    			<span><a href="" title="view incyyte" id="view_incyyte" class="button_gray" style="width:171px; "> 
					    			...
					    			<a class="report_abuse"></a>Share on :
									<a style="padding-left: 15px;" class="facebook"><img src="${pageContext.request.contextPath}/ui/images/fb_icon.png" /></a>
									<a style="padding-left: 10px;" class="twitter"><img src="${pageContext.request.contextPath}/ui/images/twitter_icon.png" /></a>
									</span>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  		<tr>
					      			...
					      			<c:set var="url"  value="${myIncoming.incyyte.uploadLocation}"/>
					      				...
					      				<div class="media_thumbs" >
														<ul id="view_videos">
										  					...
										  					<!-- this is for flow player for questions -->
															<a style="cursor:pointer"   style="display:block;width:425px;height:300px;"></a>
															<span><img width="230" height="171" alt="tour" style="margin:0px 0px 0px 80px" onClick="javascript:playYoutubeVideoQues('${myIncoming.incyyte.id}' , '${myIncoming.incyyte.youtubeUrl}');" src="ui/images/video_thumb.png" /></span>
															<div style="display:none;">
					    		...
					    		<td width="40%" valign="top" class="grid_6b">
					    		     <form:form id="voteForm${myIncoming.incyyte.id}" commandName="voteForm"  action="voteincome.cyt" method="post"> 
					    			<input type="hidden" name="questionId" id="questionId" value="${myIncoming.incyyte.id}"/>
					    			 <c:if test="${userLocal1 != null}">
	                                  <input type="hidden" name="memberId" id="memberId" value="<%userLocal1.getId();%>"/>
	                                   </c:if>
                                        <form:hidden path="incyyteId" id="incyyteId${myIncoming.incyyte.id}"/> 
                                         ...
                                         <table width="100%" border="0" cellspacing="0" cellpadding="0">
											...
											<c:forEach var="ans" items="${myIncoming.incyyte.answers}" varStatus="ansStatus">
												<tr>
												  	...
												  	<td><label for="radio-${ans.id}" tabindex="${ans.id}">${ans.answerOption}</label>		                            		
					                            		<form:radiobutton path="selectedAnswer" id="radio-${ans.id}"  name="selectedAnswer" value="${ans.id}" />
					                            	</td>
												</tr>
											</c:forEach>
											<tr>
												<td>&nbsp;</td>
												<td><span id="ans_sel_error_${myIncoming.incyyte.id}" class="error" style="font-size: small; color: red;"></span></td>
											</tr>
											<tr>
											  	<td>&nbsp;</td>
											  	<td><a href="#" onclick="submitVoteForm('voteForm${myIncoming.incyyte.id}','${myIncoming.incyyte.id}');" title="READY? VOTE!" id="ready_vote" class="ready_vote_bot" style="width:171px; margin-top:10px;">
							 ...
							 <table style="width: 50%; border-collapse:collapse;" cellspacing="0"> 
							<tr>
							<td style="width: 90px;"> <a  class="report_abuse"></a>Share on :</td> 
							<td> <a  class="facebook"><img src="${pageContext.request.contextPath}/ui/images/fb_icon.png" /></a> </td>
							<td> <a  class="twitter"><img src="${pageContext.request.contextPath}/ui/images/twitter_icon.png" /></a> </td>
						    <td ><a href="#" class="report_abuse" onclick="submitReportAbuse(${myIncoming.incyyte.id}, 'filteredPolls.cyt');">Report Abuse :<img src="${pageContext.request.contextPath}/ui/images/reportabuse_flag.png" /></a></td>
							</tr>
						</table>
				  	</div>
	  	</c:forEach>
	...
	<!-- Pagination start---->
	<c:if test="${paginationNeeded == 'Y'}">
	<c:if test="${not empty text}">
	<div class='pagination'>
	    <%--For displaying Previous link except for the 1st page --%>
	    <c:if test="${currentPage != 1}">
	        <a class='prev page-numbers'
	           href="filteredPolls.cyt?page=${currentPage - 1}&search=${text}">Prev</a>
	    </c:if>
	    <%--For displaying Page numbers.
	    The when condition does not display a link for the current page--%>
	     
	    <c:forEach begin="1" end="${filterNoOfPages}" var="i">
	        <c:choose>
	            <c:when test="${currentPage eq i}">
	                <span class='page-numbers currentPage'>${i}</span>
	            </c:when>
	            <c:otherwise>
	                <c:if test="${i <= 5}">
	                    <a class='page-numbers'
	                       href="filteredPolls.cyt?page=${i}&search=${text}">${i}</a>
	                </c:if>
	                <c:if test="${i > 5}">
	                    <c:if test="${i+1 == filterNoOfPages}">
	                        <a class='page-numbers'>..</a>
	                        <a class='page-numbers'
	                           href="filteredPolls.cyt?page=${i}&search=${text}">${i}</a>
	                    </c:if>
	                    <c:if test="${i == filterNoOfPages}">
	                        <a class='page-numbers'
	                           href="filteredPolls.cyt?page=${i}&search=${text}">${i}</a>
	                    </c:if>
	                </c:if>
	    ...
	    <c:if test="${currentPage lt filterNoOfPages}">
	        <a class='next page-numbers'
	           href="filteredPolls.cyt?page=${currentPage + 1}&search=${text}">Next</a>
	    </c:if>
	</div>
  </c:if>
...
<div class='pagination'>
	    ...
	    <c:if test="${currentPage != 1}">
	        <a class='prev page-numbers'
	           href="filteredPolls.cyt?page=${currentPage - 1}&category=${category}">Prev</a>
	    </c:if>
	    <%--For displaying Page numbers.
	    The when condition does not display a link for the current page--%>
	    ...
	    <c:forEach begin="1" end="${filterNoOfPages}" var="i">
	        <c:choose>
	            ...
	            <c:otherwise>
	                <c:if test="${i <= 5}">
	                    <a class='page-numbers'
	                       href="filteredPolls.cyt?page=${i}&category=${category}">${i}</a>
	                </c:if>
	                <c:if test="${i > 5}">
	                    <c:if test="${i+1 == filterNoOfPages}">
	                        <a class='page-numbers'>..</a>
	    ...
	    <c:if test="${currentPage lt filterNoOfPages}">
	        <a class='next page-numbers'
	           href="filteredPolls.cyt?page=${currentPage + 1}&category=${category}">Next</a>
	    </c:if>
	</div>
</c:if>
 ...
 <style>
  ...
  function submitVoteForm(formId, incyyteId){
	  //Assigning selected answer to hidden var
	   $("#selectedAnsVal").val($("input:radio[name='selectedAnswer']:checked").val());
	//  alert($("#selectedAnsVal").val());
	  $('#questionId').val(incyyteId);
	 $('#incyyteId' + incyyteId).val(incyyteId);
	// alert("incyyteId::" + incyyteId);
	// alert("incyyteId::" + $('#incyyteId' + incyyteId).val());
      ...
      $("#ans_sel_error_" + incyyteId).css("display", "none");
        ...
        if($("input:radio[name='selectedAnswer']:checked").val() != null){
            $("#"+formId).ajaxSubmit({
                type: 'POST',
                url: '/filteredvoteloggedinUser.cyt',
                success: function(data) {
                    if(data.indexOf("notLogin") != -1) {
        				loginProcessInModelLoginPage('filteredPoll', incyyteId);
