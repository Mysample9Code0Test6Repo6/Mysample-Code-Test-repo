...
<script src="ui/js/dashboard/dash_region.js" type="text/javascript"></script>
...
/* $(window).scroll(function(){
	if($(window).scrollTop() >= $(document).height() - $(window).height() - 10){
		...
		$.ajax({
			type:'POST',
			url: "loadregionIncyyte.cyt",
			success: function(data){
				if (data.indexOf("success") != -1) {
					$(window).scrollTop(101);
					$("#region_incyyte").load('displayregionIncyyte.cyt');	
...
<div id="most_resent_incyyte">
	<div id="region_incyyte">
	
		<c:forEach var="myRegion" items="${regionIncyytes}" varStatus="status">
			
			<% InCyyteChart cyyteChart = (InCyyteChart)pageContext.getAttribute("myRegion");%>
			...
			<c:choose>
				<c:when test="${myRegion.incyyteClosed}">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  		<tr>
					    		<td width="12%" valign="top" >
					    			<c:choose>
					    				<c:when test="${not empty myRegion.incyyte.senderProfilePic && myRegion.incyyte.createdBy != 'ANONYMOUS'}">
					    		...
					    		<td width="58%"  >
					    			<h3><p class="questionText">${myRegion.question}</p></h3>
					      			<p>Sender: <strong>${myRegion.incyyte.createdBy}</strong></p>
					      			<p>Created:  <strong>${myRegion.incyyte.createdDatePeriod}</strong></p>
					      			<p>Closure:${myRegion.incyyte.closureDate}</p>
					      			<c:if test="${not empty myRegion.incyyte.group}">
										<p>Group: ${myRegion.incyyte.group.groupName}</p>	
									</c:if>	
									<BR>			      			    		
					      		</td>	
			      				<td width="30%">
							      	<p style="float:right;margin-top:-15px;margin-right:5px;" align="center"><span class="font_14px">VOTING <br> CLOSED!</span></p>
									<br><a href="#" onclick="deleteRegion('${myRegion.incyyte.id}');" title="Delete Incyyte" class="trash_icon"></a><a href="dashdetails.cyt?code=${myRegion.incyyte.incyyteCode}&frm=region" title="View Result" id="vote_completed" class="vote_completed vote_completed_ie8">  View Result</a>
								</td>			      			     		
					  		</tr>
						</table>
					</div>
					<%int filesCount=0;%>
					<c:forEach var="ans" items="${myRegion.incyyte.answers}" varStatus="ansStatus">
					<c:choose>
					...
					<div class="incomming_incyyte_tab2">
						<table width="100%"  border="0" cellspacing="0" cellpadding="0">
					  		<tr>
					    		...
					    		<td width="58%">
					    			<p onclick="javascript:window.location.href='dashdetails.cyt?code=${myRegion.incyyte.incyyteCode}&frm=region';"><h3><p class="questionText" >${myRegion.question}</p></h3></p>
					      			<p>Sender: <strong>${myRegion.incyyte.createdBy}</strong></p>
					      			<p>Created:  <strong>${myRegion.incyyte.createdDatePeriod}</strong></p>
					      			<p>Closure:${myRegion.incyyte.closureDate}</p>
					      			<c:if test="${not empty myRegion.incyyte.group}">
			      				...
			      				<td width="30%" >		
									...
									<br><a href="#" onclick="deleteRegion('${myRegion.incyyte.id}');" title="Delete Incyyte"><span class="trash_icon"></span></a><a href="dashdetails.cyt?code=${myRegion.incyyte.incyyteCode}&frm=region" title="View Result" id="vote_completed" class="vote_completed vote_completed_ie8">View Result</a>
								</td>
								 </tr>
						</table>
					</div>
				...
				<c:otherwise>
								   ...
								   <%--  <div id="dropBoxContainer${status.index}" class="dropBoxContainer"> 
										...
										<div id="dropBox${status.index}" class="dropBox">
									  		<div id="dropBoxContent${status.index}" class="dropBoxContent">
											    <ul>
											      <li><a href="#" onclick="delete_incyyte('${myRegion.incyyte.id}');" id="delete_incyyte" >Delete this InCyyte <span>The inCyyte will be deleted</span></a></li>
											    </ul>
									  		</div>
										</div>
									</div> --%>
					...
					<div class="incomming_incyyte_detail">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  		<tr>
					    		<td width="60%" valign="top" >
					    			<table width="100%" border="0" cellspacing="0" cellpadding="0">
					      			...
					      			<br>
					      			<c:choose>
					      			<c:when test="${not empty myRegion.incyyte.uploadLocation && not empty myRegion.incyyte.upload_name}">
					      			<c:set var="url"  value="${myRegion.incyyte.uploadLocation}"/>
					      					...
					      					<%if(extVideos.contains(extension)) {%>
							      					<div class="media_thumbs" >
														<ul id="view_videos">
															...
															<span><img onClick="javascript:playVideo('${myRegion.incyyte.id}','player${myRegion.incyyte.id}', '${myRegion.incyyte.uploadLocation}')" src="ui/images/video_thumb.png" /></span>
															<div style="display:none;">
													        <div id="emailList${myRegion.incyyte.id}" class="emailList ">
															<a style="cursor:pointer" id="player${myRegion.incyyte.id}" ></a>
													        </div></div>
										    				</li>
														</ul>
									      			</div>
												  		...
												  		<div id="videos_modal">
												    		<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0" width="480" height="360" id="FLVPlayer">
																<param name="movie" value="FLVPlayer_Progressive.swf" />
															  	...
															  	<param name="FlashVars" value="&MM_ComponentVersion=1&skinName=Clear_Skin_2&streamName=${myRegion.incyyte.uploadLocation}&autoPlay=true&autoRewind=false" />
															  	<embed src="FLVPlayer_Progressive.swf" flashvars="&MM_ComponentVersion=1&skinName=Clear_Skin_2&streamName=${myRegion.incyyte.uploadLocation}&autoPlay=true&autoRewind=false" quality="high" scale="noscale" width="480" height="360" name="FLVPlayer" salign="LT" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
															</object>
												  		</div>
												  	</div>
							      					...
							      					<div class="media_thumbs" >
														<ul id="view_photos">
															<li> 
																	<a style="cursor:pointer;" href="${myRegion.incyyte.uploadLocation}" class="fancybox-popup fancybox.iframe">
																	<img src="${myRegion.incyyte.uploadLocation}" width="230" height="171" alt="tour" style="margin:0px 0px 0px 80px"  class="photoframe"   /></a>
															</li>
													  </ul>
								      				</div>
							      				...
							      				<%}else if(extDocs.contains(extension)){ %>		
								      				<div class="media_thumbs" >
														<ul id="view_document">
															<li> 
																<a style="cursor:pointer" href="" onClick="window.open('${myRegion.incyyte.uploadLocation}','MyWindow'); return false;" target="_blank" class="group">
					      				...
					      				<c:when test="${not empty myRegion.incyyte.youtubeUrl}">
					      				<div class="media_thumbs" >
														<ul id="view_videos">
										  					<li> 
										  					<!-- this is for flow player for questions -->
															<a style="cursor:pointer"   style="display:block;width:425px;height:300px;"></a>
															<span><img onClick="javascript:playYoutubeVideoQues('${myRegion.incyyte.id}' , '${myRegion.incyyte.youtubeUrl}');" src="ui/images/video_thumb.png" /></span>
															<div style="display:none;">
																<div id="emailListYouTube_${myRegion.incyyte.id}" class="emailList " >
											   						 <iframe width='390' id="iFrameYouTube_${myRegion.incyyte.id}"  height='308'  src="ui/images/uploading_big_img.png" frameborder='0' type='text/html'></iframe>
											    			</div>
															</div> 
										    				</li>
														</ul>
					    		...
					    		<td width="40%" valign="top" class="grid_6b">
					    			<form:form id="voteForm${status.index}" commandName="voteForm"  action="voteregion.cyt" method="post">
					    			
					    				<input type="hidden" name="incyyteId" id="incyyteId" value="${myRegion.incyyte.id}"/>
							...
							<tr>
								<td colspan="3"><a href="#" onclick="submitReportAbuse(${myRegion.incyyte.id}, 'dashregion.cyt');"><span id="report_abuse" class="report_abuse"><span></span>Report Abuse</span></a></td>
							</tr>
						</table>
				  	</div>
				</c:otherwise>
	...
	<div class='pagination'>
	    ...
	    <c:if test="${currentPage != 1}">
	        <a class='prev page-numbers'
	           href="dashregion.cyt?page=${currentPage - 1}&param=${text}">Prev</a>
	    </c:if>
	    <%--For displaying Page numbers.
	    The when condition does not display a link for the current page--%>
	    <c:forEach begin="1" end="${regionnoOfPages}" var="i">
	        <c:choose>
	            ...
	            <c:otherwise>
	                <c:if test="${i <= 5}">
	                    <a class='page-numbers'
	                       href="dashregion.cyt?page=${i}&param=${text}">${i}</a>
	                </c:if>
	                <c:if test="${i > 5}">
	                    <c:if test="${i+1 == regionnoOfPages}">
	                        <a class='page-numbers'>..</a>
	                        <a class='page-numbers'
	                           href="dashregion.cyt?page=${i}&param=${text}">${i}</a>
	                    </c:if>
	                    <c:if test="${i == regionnoOfPages}">
	                        <a class='page-numbers'
	                           href="dashregion.cyt?page=${i}&param=${text}">${i}</a>
	                    </c:if>
	                </c:if>
	    ...
	    <c:if test="${currentPage lt regionnoOfPages}">
	        <a class='next page-numbers'
	           href="dashregion.cyt?page=${currentPage + 1}&param=${text}">Next</a>
	    </c:if>
	</div>
	<!-- Pagination ends---->
