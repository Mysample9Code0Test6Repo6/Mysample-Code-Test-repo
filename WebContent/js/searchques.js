...
function document_onInit()
{
	...
	//wire event handlers
	$("#SearchQues").click(function() { SearchQues(); });
...
function SearchQues(){
	 if (true)  //   if (validated()) 
	 {
		 ...
		 $("#searchQuesForm").ajaxSubmit({	
				type: 'POST',
				url: 'questions/submit.cyt',
				success: function(data) {				
					window.location = "questions/SearchResult.cyt";																						
...
function ShowQuesDtls(id,text,category){
	
	alert() ;
	 $("#searchQuesForm").ajaxSubmit({	
			type: 'GET',
			url: 'questions/getdtls.cyt',
			success: function(data) {				
				window.location = "quesdtls.cyt";																						
